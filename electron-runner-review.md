...
The concerns cluster in four areas, ordered by real impact at 200K jobs/month:

1. **Windows runners are non-ephemeral** — the biggest single deviation from the "one job, one instance" model. Reused runners inherit state, credentials, and caches from previous jobs. Combined with UAC-disabled and `ghrunner` being a full local Administrator, the Windows security posture is materially weaker than Linux (see H0, H7).
2. **Cost efficiency** — `minimum_running_time_in_minutes: 15` (Linux) / **30** (Windows) and the `sleep 10` in every post-job hook are the biggest levers in the stack. Tuning them is worth **~$1K–4K/month** on Linux alone; Windows adds more.
3. **Logging** — the AMI installs the CloudWatch agent but the Ubuntu Ansible task never configures or starts it; the philips-labs module wires it at runtime via SSM. Windows explicitly enables the agent at runtime (`amazon-cloudwatch-agent-ctl.ps1 -a fetch-config`), Linux does not. Two layers touch log shipping on Linux with no single owner — the most likely explanation for reports of missing logs.
4. **Supply chain / change control** — the runner module is pinned to a mutable branch (`pfizer-main`), the config repo pins the runner module by mutable branch (`deploy`), and that `deploy` branch is **not protected**. Two unreviewed mutable refs sit between commit and production.

Nothing here is a fire. The Windows-ephemeral, logging, and cost items are the ones worth doing this quarter.
...
- **Risk:** Every Linux runner is billed for ≥15 min, Windows for ≥30 min, even if the job took 45 seconds. At 200K jobs/month this is the single largest cost multiplier in the stack. Windows is 2× the Linux impact per job. Note also: Windows runners are non-ephemeral (H0), so this parameter drives idle-runner life rather than per-job billing — same cost effect either way, but the reasoning changes.
...
- **Priority:** HIGH.
- **Cost impact:** Illustrative — if Linux median is 4 min and 60% of Linux jobs are <15 min, dropping 15 → 5 saves ~10 min × ~120K jobs = **~20,000 instance-hours/month** on Linux alone. c6i.xlarge spot ($0.06) ≈ **~$1,200/mo**; on-demand ≈ **~$3,400/mo**. Windows dropping 30 → 15 could double this on the Windows share. Likely the highest-$-value finding in the review.
...
- **Cost levers stack.** H1 (min runtime, Linux 15 + Windows 30), H2 (sleep 10, Linux only), M9 (spot vs on-demand) are all in the "instance-hours" bucket. Together they represent the majority of the compute bill. Do H1 and H2 first (pure wins, no reliability tradeoff), then decide M9 based on measured spot-interruption rate.
...
- **What Windows share of the 200K/mo job volume actually is** — sizes the impact of H0/H1 (Windows leg)/H7. If Windows is <5% of jobs, prioritise Linux fixes first; if it's larger, H0/H7 move up.
...
- **Tune `minimum_running_time_in_minutes`** (H1) — measure job-duration distribution per OS first, then drop Linux 15 → 5 and Windows 30 → 15 as data permits. Likely $1K–4K/month recurring saving on Linux alone; more on Windows. Do after H2 for cleaner data.
